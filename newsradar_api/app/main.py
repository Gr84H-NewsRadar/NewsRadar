...
async def register(user_data: schemas.UserCreate, background_tasks: BackgroundTasks,
    ...
    user = models.User(
        ...
        last_name=user_data.last_name, organization=user_data.organization,
...
async def create_user(user_data: schemas.UserCreate, background_tasks: BackgroundTasks,
                      db: Session = Depends(get_db),
...
def create_alert(user_id: int, alert_data: schemas.AlertCreate, db: Session = Depends(get_db),
                 current_user: models.User = Depends(require_manager)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    alert = models.Alert(
        user_id=user_id, name=alert_data.name, keywords=alert_data.descriptors,
        category_code=category_code, cron_expression=alert_data.cron_expression,
        notify_email=alert_data.notify_email, notify_inbox=alert_data.notify_inbox
    )
...
def get_alert(user_id: int, alert_id: int, db: Session = Depends(get_db),
              current_user: models.User = Depends(get_current_user)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
...
def update_alert(user_id: int, alert_id: int, alert_data: schemas.AlertUpdate,
                 db: Session = Depends(get_db),
                 current_user: models.User = Depends(require_manager)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    ...
    if "rss_channel_ids" in update_data:
        ...
        if ch_ids is not None:
            channels = db.query(models.RSSChannel).filter(models.RSSChannel.id.in_(ch_ids)).all()
...
@app.get(f"{API_PREFIX}/users/{{user_id}}/alerts/{{alert_id}}/notifications",
         response_model=List[schemas.Notification], tags=["notifications"])
def list_notifications(user_id: int, alert_id: int, skip: int = 0, limit: int = 100,
                       db: Session = Depends(get_db),
                       current_user: models.User = Depends(get_current_user)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    notifs = db.query(models.Notification).filter(models.Notification.alert_id == alert_id) \
...
@app.post(f"{API_PREFIX}/users/{{user_id}}/alerts/{{alert_id}}/notifications",
          response_model=schemas.Notification, status_code=201, tags=["notifications"])
def create_notification(user_id: int, alert_id: int, notif_data: schemas.NotificationCreate,
                        db: Session = Depends(get_db),
                        current_user: models.User = Depends(require_manager)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
...
@app.get(f"{API_PREFIX}/users/{{user_id}}/alerts/{{alert_id}}/notifications/{{notification_id}}",
         response_model=schemas.Notification, tags=["notifications"])
def get_notification(user_id: int, alert_id: int, notification_id: int,
                     db: Session = Depends(get_db),
                     current_user: models.User = Depends(get_current_user)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
    n = db.query(models.Notification).filter(
        models.Notification.id == notification_id, models.Notification.alert_id == alert_id).first()
    if not n:
        raise HTTPException(status_code=404, detail="Notification not found")
    return schemas.Notification.from_orm_notification(n)


@app.put(f"{API_PREFIX}/users/{{user_id}}/alerts/{{alert_id}}/notifications/{{notification_id}}",
         response_model=schemas.Notification, tags=["notifications"])
def update_notification(user_id: int, alert_id: int, notification_id: int,
...
@app.delete(f"{API_PREFIX}/users/{{user_id}}/alerts/{{alert_id}}/notifications/{{notification_id}}",
            status_code=204, tags=["notifications"])
def delete_notification(user_id: int, alert_id: int, notification_id: int,
                        db: Session = Depends(get_db),
                        current_user: models.User = Depends(require_manager)):
    if current_user.id != user_id:
        raise HTTPException(status_code=403, detail="Not authorized")
...
def update_category(category_id: int, cat_data: schemas.CategoryUpdate, db: Session = Depends(get_db),
                    current_user: models.User = Depends(require_manager)):
    cat = db.query(models.Category).filter(models.Category.id == category_id).first()
    if not cat:
        raise HTTPException(status_code=404, detail="Category not found")
...
def create_source(source_data: schemas.InformationSourceCreate, db: Session = Depends(get_db),
                  current_user: models.User = Depends(require_manager)):
...
def update_source(source_id: int, source_data: schemas.InformationSourceUpdate,
                  db: Session = Depends(get_db),
                  current_user: models.User = Depends(require_manager)):
    source = db.query(models.InformationSource).filter(models.InformationSource.id == source_id).first()
    if not source:
...
@app.get(f"{API_PREFIX}/information-sources/{{source_id}}/rss-channels",
         response_model=List[schemas.RSSChannel], tags=["rss-channels"])
def list_rss_channels(source_id: int, db: Session = Depends(get_db),
                      current_user: models.User = Depends(get_current_user)):
    s = db.query(models.InformationSource).filter(models.InformationSource.id == source_id).first()
    if not s:
        raise HTTPException(status_code=404, detail="Information source not found")
    return db.query(models.RSSChannel).filter(models.RSSChannel.information_source_id == source_id).all()


@app.post(f"{API_PREFIX}/information-sources/{{source_id}}/rss-channels",
          response_model=schemas.RSSChannel, status_code=201, tags=["rss-channels"])
def create_rss_channel(source_id: int, channel_data: schemas.RSSChannelCreate,
                       db: Session = Depends(get_db),
                       current_user: models.User = Depends(require_manager)):
    s = db.query(models.InformationSource).filter(models.InformationSource.id == source_id).first()
    if not s:
        raise HTTPException(status_code=404, detail="Information source not found")
    cat = db.query(models.Category).filter(models.Category.id == channel_data.category_id).first()
    if not cat:
        raise HTTPException(status_code=404, detail="Category not found")
...
@app.get(f"{API_PREFIX}/information-sources/{{source_id}}/rss-channels/{{channel_id}}",
         response_model=schemas.RSSChannel, tags=["rss-channels"])
def get_rss_channel(source_id: int, channel_id: int, db: Session = Depends(get_db),
                    current_user: models.User = Depends(get_current_user)):
    ch = db.query(models.RSSChannel).filter(
        models.RSSChannel.id == channel_id, models.RSSChannel.information_source_id == source_id).first()
    if not ch:
        raise HTTPException(status_code=404, detail="RSS channel not found")
    return ch


@app.put(f"{API_PREFIX}/information-sources/{{source_id}}/rss-channels/{{channel_id}}",
         response_model=schemas.RSSChannel, tags=["rss-channels"])
def update_rss_channel(source_id: int, channel_id: int, channel_data: schemas.RSSChannelUpdate,
                       db: Session = Depends(get_db),
                       current_user: models.User = Depends(require_manager)):
    ch = db.query(models.RSSChannel).filter(
        models.RSSChannel.id == channel_id, models.RSSChannel.information_source_id == source_id).first()
...
@app.delete(f"{API_PREFIX}/information-sources/{{source_id}}/rss-channels/{{channel_id}}",
            status_code=204, tags=["rss-channels"])
def delete_rss_channel(source_id: int, channel_id: int, db: Session = Depends(get_db),
                       current_user: models.User = Depends(require_manager)):
    ch = db.query(models.RSSChannel).filter(
        models.RSSChannel.id == channel_id, models.RSSChannel.information_source_id == source_id).first()
    if not ch:
...
def get_stats(skip: int = 0, limit: int = 10, db: Session = Depends(get_db),
              current_user: models.User = Depends(get_current_user)):
    rows = db.query(models.ProcessingStats).order_by(
        models.ProcessingStats.created_at.desc()).offset(skip).limit(limit).all()
...
def create_stats(stats_data: schemas.StatsCreate, db: Session = Depends(get_db),
                 current_user: models.User = Depends(require_manager)):
...
def update_stats(stats_id: int, stats_data: schemas.StatsUpdate, db: Session = Depends(get_db),
                 current_user: models.User = Depends(require_manager)):
    s = db.query(models.ProcessingStats).filter(models.ProcessingStats.id == stats_id).first()
    if not s:
        raise HTTPException(status_code=404, detail="Stats not found")
    if stats_data.metrics:
        m = {x.name: x.value for x in stats_data.metrics}
        s.total_feeds_processed = int(m.get("total_feeds_processed", s.total_feeds_processed))
        ...
        s.total_alerts_triggered = int(m.get("total_alerts_triggered", s.total_alerts_triggered))
        s.processing_time_seconds = int(m.get("processing_time_seconds", s.processing_time_seconds))
...
def list_news(
    ...
    q: Optional[str] = Query(None, description="Busqueda full-text en titulo y descripcion"),
...
def get_keyword_synonyms(keyword: str = Query(..., description="Keyword"),
                         current_user: models.User = Depends(get_current_user)):
...
def get_dashboard_stats(db: Session = Depends(get_db),
    ...
    for cat in db.query(models.Category).all():
        c1 = db.query(models.NewsItem).filter(models.NewsItem.category_id == cat.id).count()
        if c1 > 0:
            news_by_category[cat.name] = c1
        c2 = db.query(models.Alert).filter(models.Alert.category_code == cat.code).count()
        if c2 > 0:
            alerts_by_category[cat.name] = c2
    return {
        "total_sources": total_sources, "total_news": total_news, "total_alerts": total_alerts,
        "news_by_category": news_by_category, "alerts_by_category": alerts_by_category
    }


# Word cloud endpoint (RF14)
SPANISH_STOPWORDS = {
    "de", "la", "que", "el", "en", "y", "a", "los", "las", "del", "se", "con",
    "por", "una", "su", "para", "es", "al", "lo", "como", "mas", "pero", "sus",
    "le", "ya", "o", "este", "si", "porque", "esta", "cuando", "muy", "sin",
    "sobre", "tambien", "me", "hasta", "donde", "quien", "desde", "todo", "nos",
    ...
    "algunas", "algo", "nosotros", "the", "and", "of", "to", "in", "for",
    "on", "with", "as", "is", "was", "by", "are", "be", "this", "an", "or",
    "from", "at", "it", "have", "has", "but", "not", "all", "their"
}

@app.get(f"{API_PREFIX}/dashboard/wordcloud", tags=["statistics"])
def get_wordcloud(category_id: Optional[int] = None, top: int = 30,
                  db: Session = Depends(get_db),
                  current_user: models.User = Depends(get_current_user)):
...
async def trigger_rss_processing(db: Session = Depends(get_db),
                                 current_user: models.User = Depends(require_manager)):
    try:
