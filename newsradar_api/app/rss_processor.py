            ...
            for item_data in news_items:
                ...
                if existing:
                    ...
                    if existing.alert_id is None:
                        match_news_against_alerts(db, existing, item_data, alert_matches, stats)
                    continue
                ...
                match_news_against_alerts(db, news_item, item_data, alert_matches, stats)
...
def match_news_against_alerts(db: Session, news_item: models.NewsItem, item_data: Dict, alert_matches: Dict, stats: Dict):
    ...
    for alert in alerts:
        ...
        if alert.category_code and news_item.category_id:
            ...
            valid_category = db.query(models.Category).filter(models.Category.code == alert.category_code).first()
            if not valid_category:
                logger.warning(f"Alert {alert.id} has invalid category_code '{alert.category_code}', ignoring category filter")
            else:
...
async def send_cycle_notification(db: Session, alert: models.Alert, matched_news: List[models.NewsItem], stats: Dict):
    ...
    sources = list({n.rss_channel.information_source.name for n in matched_news if n.rss_channel and n.rss_channel.information_source})
    ...
    statistics = {
        ...
        'matched_keywords': list({kw for n in matched_news if n.matched_keywords for kw in n.matched_keywords}),
        'sources': sources,
    }
